...
struct Net : torch::nn::Module {
	Net(int in_dim, int out_dim) {
		...
		fc2 = register_module("fc2", torch::nn::Linear(500, 300));
		fc3 = register_module("fc3", torch::nn::Linear(300, 200));
		fc4 = register_module("fc4", torch::nn::Linear(200, 100));
		fc5 = register_module("fc5", torch::nn::Linear(100, out_dim));
	}
...
struct Net : torch::nn::Module {
	Net(int in_dim, int out_dim) {
    		...
    		int hidden_size = 20;
		//lstm = register_module("lstm", torch::nn::LSTM(torch::nn::LSTMOptions(inputs, hidden).layers(layers)));
		lstm1 = register_module("lstm1", torch::nn::LSTM(torch::nn::LSTMOptions(in_dim, hidden_size).num_layers(10).batch_first(false)));
		fc2 = register_module("fc2", torch::nn::Linear(hidden_size, out_dim));
	}
	...
	torch::Tensor forward(torch::Tensor x) {
		...
		torch::Tensor output;
        	...
        	//lstm_output = lstm1->forward(x);
        	auto lstm_output = lstm1->forward(x);
        	output = torch::relu(x);
        	//std::cout<<"NO"<<std::endl;
        	output = std::get<0>(lstm_output);
...
//std::pair<std::vector<float>, std::vector<int>> process_data(std::ifstream& file) {
	...
	for (std::size_t i = 0; i < features.size(); i++) {
		...
		inputs.insert(inputs.end(), features[i].begin(), features[i].begin()+4);
		
	}
	return std::make_pair(inputs, label);
}

std::vector<float> test_process_data(std::ifstream& file) {
//std::pair<std::vector<float>, std::vector<int>> process_data(std::ifstream& file) {
	std::vector<std::vector<float>> features;
